...
// Nombres amigables para la respuesta — el consumidor de este módulo no
...
export interface MacrosPlan {
  ...
  porDia: {
    ...
    diaSemana: string
    tipoComida: string
    totales: ReturnType<typeof aFormatoAmigable> & ReturnType<typeof aSinDatoAmigable>
    itemsSinDatosNutricionales: string[]
  }[]
  total: ReturnType<typeof aFormatoAmigable> & ReturnType<typeof aSinDatoAmigable>
  avisos: { itemId: string; motivo: string }[]
  porDiaAgregado: {
    diaSemana: string
    totales: ReturnType<typeof aFormatoAmigable> & ReturnType<typeof aSinDatoAmigable>
  }[]
...
/**
 * Calcula los macros reales (energía/proteínas/grasas/carbohidratos) de un plan
 * alimentario, por comida, por día y totales/promedio del plan — a partir de
 * los ítems 'alimento' (fuente 'argenfood', únicas con datos nutricionales
 * cargados hoy) resueltos contra vademecum_alimento_nutrientes.
 * Devuelve null si el plan no existe o no pertenece al terapeuta.
 */
export async function calcularMacrosPlan(
  db: ReturnType<typeof serviceClient>,
  planId: string,
  terapeutaId: string,
): Promise<MacrosPlan | null> {
  ...
  if (alimentoIdsArgenfood.size > 0) {
    ...
    if (valoresError) {
      console.error('[calcularMacrosPlan] Error consultando vademecum_alimento_nutrientes:', valoresError)
    }
  ...
  // Por-nutriente: a diferencia de un todo-o-nada, acá cada macro se resuelve
  // individualmente — un ítem puede tener kcal y proteínas pero no carbohidratos
  // (falta la fila puntual en vademecum_alimento_nutrientes).
