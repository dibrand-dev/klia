...
function PacienteCard({ paciente }: { paciente: PacienteListado }) {
  ...
  return (
    <>
    <Link href={`/pacientes/${paciente.id}`} className="dir-pcard">
        ...
        <div ref={menuRef} className="dir-menu-wrap">
          ...
          {menuOpen && (
            <div className="dir-menu">
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setMenuOpen(false); router.push(`/pacientes/${paciente.id}?edit=1`) }}>
              ...
              {!esColaborador && (
                <button className="danger" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setMenuOpen(false); setConfirmOpen(true) }}>
